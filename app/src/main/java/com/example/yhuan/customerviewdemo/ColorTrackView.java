...
public class ColorTrackView extends View {
    ...
    private static final int DIRECTION_SIDE = 4;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        } else if (mDirection == DIRECTION_SIDE) {
            drawOriginSide(canvas, r);
            drawChangeSide(canvas, r);
        } else if (mDirection == DIRECTION_CENTER) {
            drawOriginCenter(canvas, r);
            drawChangeCenter(canvas, r);
        }

    }

    private void drawChangeCenter(Canvas canvas, int r) {
        int halfTextWidth = mTextWidth / 2;
        drawText_h(canvas, mTextChangeColor, (int) (mTextStartX + (1 - mProgress) * halfTextWidth),
                (int) (mTextStartX + (1 + mProgress) * halfTextWidth));
    }

    private void drawOriginCenter(Canvas canvas, int r) {
        int halfTextWidth = mTextWidth / 2;
        drawText_h(canvas, mTextOriginColor, mTextStartX, (int) (mTextStartX + (1 - mProgress) * halfTextWidth));
        drawText_h(canvas, mTextOriginColor, (int) (mTextStartX + (1 + mProgress) * halfTextWidth), mTextStartX + mTextWidth);
    }

    private void drawChangeSide(Canvas canvas, int r) {
        float progress = mProgress / 2;
    ...
    private void drawOriginSide(Canvas canvas, int r) {
        float progress = mProgress / 2;
        drawText_h(canvas, mTextOriginColor, (int) (mTextStartX + mProgress * mTextWidth),
    ...
    protected Parcelable onSaveInstanceState() {
        ...
        bundle.putParcelable(KEY_DEFAULT_STATE, super.onSaveInstanceState());//父类可能保存了某种状态，可能是有意义的
        return bundle;
    }
    ...
    protected void onRestoreInstanceState(Parcelable state) {
        ...
        if (state instanceof Bundle) {
            ...
            super.onRestoreInstanceState(bundle
                    .getParcelable(KEY_DEFAULT_STATE));//获取到之前存储的父类状态，再交由父类处理
            return;
        }
